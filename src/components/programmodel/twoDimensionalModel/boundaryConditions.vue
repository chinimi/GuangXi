    ...
    <div class="boundaryConditions_top">
      <div class="boundaryConditions_left">
        <div v-for="(item, index) in data" :key="index">
          ...
          :style="ModelSubType == 'Scenario' && item.name !='污染源' && item.name !='降雨站' || ModelSubType == 'EmerMnt' && item.name !='水质边界' && item.name !='降雨站'  ? 'display: block;' : 'display: none;'"
          ...
          >
           <li
            ...
            >
              {{i.BoundaryName}}
              <span v-if="item.id == 5"><i class="el-icon-delete" @click="deleteSourceInformation(index,i,inx)"></i></span>
            </li>
          </ul>
        </div>
      </div>
      ...
      <div v-if="sourcePollution">
        <div class="boundaryConditions_content" style="border: 1px solid #EBEEF5;padding-top:10px;">
          ...
          <div class="singleli_title">
            <el-row>
              ...
              <el-col :span="14">
                <div>
                <el-input style="width:180px" v-model="coordinate" placeholder="请输入内容" disabled></el-input>
                </div>
              </el-col>
            </el-row>
          </div>
          <!-- <div class="singleli_title">
            <el-row>
              ...
              <el-col :span="14">
                <div>
                <el-input style="width:180px" v-model="riverMiles" placeholder="请输入内容" disabled></el-input>
                </div>
              </el-col>
            </el-row>
          </div> -->
        ...
        <div class="boundaryConditions_right" style="border: 0">
          ...
          <div class="boundaryConditions_bottom">
          <el-button size="small" plain @click="preserveSourceInformation">保存</el-button>
          <el-button size="small" plain>计算</el-button>
          <el-button size="small" plain>查看结果</el-button>
      </div>
        </div>
...
export default {
   ...
   tap_info(index,item,e){
      ...
      this.Title = item.BoundaryName
       if(this.$route.params.value != undefined){
         ...
         if(this.name == '流量边界' || this.name == '水位边界' || this.name == '降雨站'){
        var url =
        modelURL +
      ...
      fetch(url)
        ...
        var url =
        modelURL +
        "/api/GXRCWQ/ModelManager/GetPollutionBoundaryInfo?scenarioCode="+this.ScenarioCode+"&sourceName="+this.boundaryId
      fetch(url)
        .then(respose => {
          return respose.json();
        })
        .then(data => {
          ...
          this.riverMiles = data.BranchName +','+ data.Chainage.toFixed(1)
    ...
    deleteSourceInformation(index,item,e){
      ...
      var url =
        modelURL +
        "/api/GXRCWQ/ModelManager/DeletePollutionBoundaryInfo?scenarioCode="+this.ScenarioCode+"&sourceName="+item.BoundaryId;
      fetch(url)
        .then(respose => {
          return respose.json();
        })
  ...
  //获取数据
      ...
      var url = modelURL + "/api/GXRCWQ/ModelManager/UpdateAndCreatePollutionBouncaryInfo";
      var _this = this;
      $.ajax({
        type: "post",
        dataType: "json",
