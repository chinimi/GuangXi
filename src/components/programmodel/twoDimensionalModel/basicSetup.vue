    ...
    <div class="basicSetup_right">
      <div class="basicSetup_textarea">
        ...
        <div style="margin: 10px;width:45%;float: left;">
          <el-progress :percentage="scheduleValue" v-if="state"></el-progress>
        </div>
        <div style="margin: 10px;float: right;width:39%">
          <el-button type="primary" size="small" plain @click="saveModel"
            >保存</el-button
          >
          <el-button type="primary" size="small" plain @click="calculateData"
            >计算</el-button
          >
  ...
  methods: {
    ...
    getModelManagerData() {
      if (this.$route.params.value != undefined) {
        ...
        var url =
          modelURL +
          "/api/GXRCWQ/ModelManager/GetScenarioInfo?scenarioCode=" +
          this.ScenarioCode;
        fetch(url)
          .then(respose => {
            return respose.json();
    ...
    calculateData() {
      if (this.$route.params.value != undefined) {
        ...
        var promise = new Promise((resolve, reject) => {
          var url =
            modelURL +
            "/api/GXRCWQ/ModelManager/PrepareRunScenario?scenarioCode=" +
            this.ScenarioCode;
          fetch(url)
            .then(respose => {
              return respose.json();
    ...
    fun(timer) {
      setTimeout(() => {
        var url =
          modelURL +
          "/api/GXRCWQ/ModelManager/GetModelRunState?scenarioCode=" +
          this.ScenarioCode;
        fetch(url)
          .then(respose => {
            return respose.json();
          ...
          .then(data => {
            this.scheduleValue = data.Progress;
            this.Message = data.Message;
          });
        if (this.scheduleValue == 100) {
          this.state = false;
          this.$message({
    ...
    async importFile(file) {
      ...
      const isSubmit = await this.$confirm("是否继续上传文件?", "提示", {
        ...
        .then(() => {
          ...
          var url =
            modelURL +
            "/api/GXRCWQ/ModelManager/UploadScenairoDescriptionDocxFile";
          console.log(url);
          $.ajax({
            type: "post",
            dataType: "json",
    ...
    downloadFile() {
      if (this.$route.params.value != undefined) {
        ...
        var url =
          modelURL +
          "/api/GXRCWQ/ModelManager/DownloadScenairoDescriptionDocxFile?scenarioCode=" +
          this.ScenarioCode;
