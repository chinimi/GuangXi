<template>
  ...
  <div class="boundaryConditions">
    <div class="boundaryConditions_top">
      <div class="boundaryConditions_left">
        <div v-for="(item, index) in data" :key="index">
          <div class="data_info" @click="tap(item, index)">
            <span >{{ item.name }}</span>
            <span v-if="item.id == 5" style="padding-left:50px">添加</span>
            <!-- 判断为污染源的是显示添加 -->
            <i
          ...
          <div>
            ...
            >
              <li
                class="info"
                v-for="(items, indexs) in item.DischargeItems"
                :key="indexs"
                :class="active == indexs ? 'isactive' : ''"
                @click="tap_info(items, indexs, $event)"
              >
                {{ items.BoundaryName }}
                <span v-if="item.id == 5">图片</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div
        class="boundaryConditions_right"
        v-if="sourcePollution"
        style="border: 0"
      >
        <el-table
...
export default {
  ...
  methods: {
    ...
    tap_info(item, index, e) {
      ...
      var url =
        modelURL +
        "/api/GXRCWQ/ModelManager/GetBoundaryTSData?scenarioCode=DHJKTXRCFA&boundaryId=" +
        boundaryId +
        "&componentName=" +
        this.componentName;
      fetch(url)
    ...
    getTableData() {
      ...
      fetch(url)
        ...
        .then(data => {
          this.data[0].DischargeItems = data.DischargeItems;
          this.data[1].DischargeItems = data.WaterLevelItems;
          this.data[2].DischargeItems = data.WaterQualityItems;
          this.data[3].DischargeItems = data.RainfallItems;
          // this.data[4].DischargeItems = data.PollutionSourceItems;
          this.data[4].DischargeItems = [
            {"BoundaryType":0,"BoundaryId":"10000001","BoundaryName":"富藏","ComponentName":null,"StationCode":"10000001","TSData":null},
            {"BoundaryType":0,"BoundaryId":"10000002","BoundaryName":"六陈","ComponentName":null,"StationCode":"10000002","TSData":null},
            {"BoundaryType":0,"BoundaryId":"10000003","BoundaryName":"大湟江口","ComponentName":null,"StationCode":"10000003","TSData":null}
            ];
        });
    },
    drawLine(data) {
